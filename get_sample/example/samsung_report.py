...
class SamsungReport : 
    ...
    def noun_embedding(self):
        ...
        for token in tokens:
            ...
            if len(''.join(_)) > 1: # 명사가 있는 것만 nouns에 담음.
    ...
    def draw_wordcloud(self):
        ...
        print(freqtext[:10])
        wcloud = WordCloud(f'{self.data.dname}D2Coding.ttf', relative_scaling=0.2, background_color='white').generate(' '.join(self.morphemes))
...
punkt 또한 문장 구조를 학습한 일종의 모델로, 
어떤 것이 약어에 쓰이는 "."이고(Ex : Ph.D.), 어떤 것이 마침표인지 학습이 되어있다. 
문장을 기본적으로 마침표를 기준으로 나누되, 
Ph.D., Saint., Professor., 와 같은 약어(Abbreviation)는 Known abbreviation으로 학습하여 
한 단어로 취급하는 방식이다.
